layui.define(["jquery","layer"], function (exports) {
	...
	theme.colorSet = function(color) {
		...
		let style = '';
		style += '.light-theme .wayne-nav-tree .layui-this a:hover,.light-theme .wayne-nav-tree .layui-this,.light-theme .wayne-nav-tree .layui-this a,.wayne-nav-tree .layui-this a,.wayne-nav-tree .layui-this{background-color: ' +color + '!important;}';
		style += '.wayne-admin .layui-logo .title{color:' + color + '!important;}';
		style += '.wayne-frame-title .dot,.wayne-tab .layui-this .wayne-tab-active{background-color: ' + color +'!important;}';
		style += '.bottom-nav li a:hover{background-color:' + color + '!important;}';
		style += '.wayne-admin .layui-header .layui-nav .layui-nav-bar{background-color: ' + color + '!important;}'
		...
		style += '.wayneone-color .color-content li.layui-this:after, .wayneone-color .color-content li:hover:after {border: ' +color + ' 3px solid!important;}';
		style += '.layui-nav .layui-nav-child dd.layui-this a, .layui-nav-child dd.layui-this{background-color:' + color + '!important}';	
		style += '.wayne-social-entrance {background-color:' + color + '!important}';
		style += '.wayne-admin .pe-collaspe {background-color:' + color + '!important}';
		style += '.layui-fixbar li {background-color:' + color + '!important}';
		if(this.autoHead){
			style += '.wayne-admin .layui-header{background-color:' + color + '!important;}.wayne-admin .layui-header .layui-nav .layui-nav-item>a{color:white!important;}';
		}
		style += '.wayne-btn-primary {background-color:' + color + '!important}';
		...
		style += '.wayne-text{color:' + color + '!important}';
		style += '.wayne-collasped-pe{background-color:'+color+'!important}'
		...
		style += '.wayne-tab-menu .item:hover{background-color:'+color+'!important}'

		let colorPane = $("#wayne-admin-color");
		if(colorPane.length>0){
			colorPane.html(style);
		}else{
			$("head").append("<style id='wayne-admin-color'>"+style+"</style>")
		}
	}
