layui.define(['table', 'jquery', 'element'], function (exports) {
    ...
    var wayneMenu = function (opt) {
        this.option = opt;
    };

    wayneMenu.prototype.render = function (opt) {
        ...
        return new wayneMenu(opt);
    }

    wayneMenu.prototype.click = function (clickEvent) {
        var _this = this;
        $("body").on("click", "#" + _this.option.elem + " .site-demo-active", function () {
            ...
            if ($("#" + _this.option.elem).is(".wayne-nav-mini")) {
    ...
    wayneMenu.prototype.skin = function (skin) {
        var menu = $(".wayne-nav-tree[lay-filter='" + this.option.elem + "']").parent();
        menu.removeClass("dark-theme");
        menu.removeClass("light-theme");
        menu.addClass(skin);
    }

    wayneMenu.prototype.selectItem = function (wayneId) {
        if (this.option.control != false) {
            $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parents(".layui-side-scroll ").find("ul").css({
                display: "none"
            });
            $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parents(".layui-side-scroll ").find(".layui-this").removeClass(
                "layui-this");
            $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parents("ul").css({
                display: "block"
            });
            var controlId = $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parents("ul").attr("wayne-id");
            if (controlId != undefined) {
                $("#" + this.option.control).find(".layui-this").removeClass("layui-this");
                $("#" + this.option.control).find("[wayne-id='" + controlId + "']").addClass("layui-this");
            }
        }
        if (this.option.accordion == true) {
            $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parents(".wayne-nav-tree").find(".layui-nav-itemed").removeClass(
                "layui-nav-itemed");
        }
        $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parents(".wayne-nav-tree").find(".layui-this").removeClass(
            "layui-this");
        if (!$("#" + this.option.elem).is(".wayne-nav-mini")) {
            $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parents(".layui-nav-item").addClass("layui-nav-itemed");
            $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parents("dd").addClass("layui-nav-itemed");
        }
        $("#" + this.option.elem + " a[menu-id='" + wayneId + "']").parent().addClass("layui-this");
    }

    var activeMenus;
    wayneMenu.prototype.collaspe = function (time) {
        ...
        if ($("#" + this.option.elem).is(".wayne-nav-mini")) {
            ...
            $("#" + this.option.elem).removeClass("wayne-nav-mini");
            ...
            $("#" + this.option.elem).addClass("wayne-nav-mini");
            $("#" + this.option.elem).animate({
                width: "60px"
            }, 400);
    ...
    function createMenu(option) {
        var menuHtml = '<div style="height:100%!important;" class="wayne-side-scroll layui-side-scroll ' + option.theme + '"><ul lay-filter="' + option.elem +
            '" class="layui-nav arrow   wayne-menu layui-nav-tree wayne-nav-tree">'
    ...
    function createMenuAndControl(option) {
        var control = '<ul class="layui-nav  wayne-nav-control pc layui-hide-xs">';
        var controlPe = '<ul class="layui-nav wayne-nav-control layui-hide-sm">';
        // 声 明 头 部
        ...
        $.each(option.data, function (i, item) {
            ...
            if (i === option.defaultMenu) {
                controlItem = '<li wayne-href="' + item.href + '" wayne-title="' + item.title + '" wayne-id="' + item.id +
                    '" class="layui-this layui-nav-item"><a href="#">' + item.title + '</a></li>';
                menuItem = '<ul  wayne-id="' + item.id + '" lay-filter="' + option.elem +
                    '" class="layui-nav arrow layui-nav-tree wayne-nav-tree">';
                // 兼容移动端
                controlPe += '<li class="layui-nav-item"><a class="pe-title" href="javascript:;" >' + item.title + '</a>';
                controlItemPe += '<dd  wayne-href="' + item.href + '" wayne-title="' + item.title + '" wayne-id="' + item.id +
                    '"><a href="javascript:void(0);">' + item.title + '</a></dd>';
            } else {
                controlItem = '<li  wayne-href="' + item.href + '" wayne-title="' + item.title + '" wayne-id="' + item.id +
                    '" class="layui-nav-item"><a href="#">' + item.title + '</a></li>';
                menuItem = '<ul style="display:none" wayne-id="' + item.id + '" lay-filter="' + option.elem +
                    '" class="layui-nav arrow layui-nav-tree wayne-nav-tree">';
        ...
        $("#" + option.control + " .wayne-nav-control").on("click", "[wayne-id]", function () {
            $("#" + option.elem).find(".wayne-nav-tree").css({
                display: 'none'
            });
            $("#" + option.elem).find(".wayne-nav-tree[wayne-id='" + $(this).attr("wayne-id") + "']").css({
                display: 'block'
            });
            $("#" + option.control).find(".pe-title").html($(this).attr("wayne-title"));
            $("#" + option.control).find("")
            option.change($(this).attr("wayne-id"), $(this).attr("wayne-title"), $(this).attr("wayne-href"))
        })
    }
    ...
    function downShow(option) {
        $("body #" + option.elem).on("click", "a[menu-type='0']", function () {
            if (!$("#" + option.elem).is(".wayne-nav-mini")) {
    ...
    function isHoverMenu(b, option) {
        if (b) {
            $("#" + option.elem + ".wayne-nav-mini .layui-nav-item,#" + option.elem + ".wayne-nav-mini dd").hover(function (e) {
                e.stopPropagation();
                var _this = $(this);
    ...
    exports(MOD_NAME, new wayneMenu());
})