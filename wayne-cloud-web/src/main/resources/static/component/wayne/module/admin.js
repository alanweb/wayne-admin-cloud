layui.define(['message','jquery', 'yaml','form', 'tab', 'menu', 'frame', 'theme'],
	function(exports) {
		...
		const $ = layui.jquery,
			...
			wayneTab = layui.tab,
			wayneMenu = layui.menu,
			wayneFrame = layui.frame,
			wayneTheme = layui.theme;
		...
		const wayneAdmin = new function() {

			// 默认配置
			let configType = 'yml';
			let configPath = 'wayne.config.yml';
			...
			this.render = function(initConfig) {
				if (initConfig !== undefined) {
					...
					applyConfig(wayneAdmin.readConfig());
				}
			}
			...
			this.themeRender = function(option) {
				...
				wayneTheme.changeTheme(window, option.other.autoHead);
			...
			this.menuSkin = function(theme) {
				const wayneAdmin = $(".wayne-admin");
				wayneAdmin.removeClass("light-theme");
				wayneAdmin.removeClass("dark-theme");
				wayneAdmin.addClass(theme);
			}
		...
		body.on("click", ".collaspe,.wayne-cover", function() {
			collaspe()
		});
		...
		function bottomTool(){
			return "<button id='closeTheme' style='position: absolute;bottom: 20px;left: 20px;' class='wayne-btn'>关闭</button>"
		}
		...
		body.on('click', '.select-color-item', function() {
			...
			localStorage.setItem("theme-color", currentColor.id);
			localStorage.setItem("theme-color-context", currentColor.color);
			wayneTheme.changeTheme(window, config.other.autoHead);
		});

		function applyConfig(param) {
			config = param;
			wayneAdmin.logoRender(param);
			wayneAdmin.menuRender(param);
			wayneAdmin.bodyRender(param);
			wayneAdmin.themeRender(param);
			wayneAdmin.keepLoad(param);
			if(param.header.message!=false){
				wayneAdmin.messageRender(param);
			}
		}
