layui.define(['jquery', 'element'], function (exports) {
    ...
    wayneTab.prototype.render = function (opt) {
        //默认配置值
        var option = {
            elem: opt.elem,
            data: opt.data,
        ...
        if (option.session) {
            // 替换 opt.data 数据
            if (sessionStorage.getItem(option.elem + "-wayne-tab-data") != null) {
                tabData = JSON.parse(sessionStorage.getItem(option.elem + "-wayne-tab-data"));
                option.data = JSON.parse(sessionStorage.getItem(option.elem + "-wayne-tab-data"));
                tabDataCurrent = sessionStorage.getItem(option.elem + "-wayne-tab-data-current");
        ...
        option.success(sessionStorage.getItem(option.elem + "-wayne-tab-data-current"));
        ...
        return new wayneTab(option);
    }

    wayneTab.prototype.click = function (callback) {
        var elem = this.option.elem;
        var option = this.option;
        element.on('tab(' + this.option.elem + ')', function (data) {
            var id = $("#" + elem + " .layui-tab-title .layui-this").attr("lay-id");
            sessionStorage.setItem(option.elem + "-wayne-tab-data-current", id);
            callback(id);
        });
    }

    wayneTab.prototype.positionTab = function () {
    ...
    wayneTab.prototype.clear = function () {
        sessionStorage.removeItem(this.option.elem + "-wayne-tab-data");
        sessionStorage.removeItem(this.option.elem + "-wayne-tab-data-current");
    }

    wayneTab.prototype.addTab = function (opt) {
        var title = '';
        if (opt.close) {
            title += '<span class="wayne-tab-active"></span><span class="able-close">' + opt.title +
                '</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
        } else {
            title += '<span class="wayne-tab-active"></span><span class="disable-close">' + opt.title +
                '</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
        }
        ...
        tabData.push(opt);
        sessionStorage.setItem(this.option.elem + "-wayne-tab-data", JSON.stringify(tabData));
        sessionStorage.setItem(this.option.elem + "-wayne-tab-data-current", opt.id);
    ...
    wayneTab.prototype.delTabByElem = function (elem, id, callback) {
    ...
    wayneTab.prototype.delCurrentTabByElem = function (elem, callback) {
    ...
    wayneTab.prototype.addTabOnlyByElem = function (elem, opt, time) {
        var title = '';
        if (opt.close) {
            title += '<span class="wayne-tab-active"></span><span class="able-close">' + opt.title +
                '</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>'
        ...
        if ($(".layui-tab[lay-filter='" + elem + "'] .layui-tab-title li[lay-id]").length <= 0) {
            ...
            if (time != false && time != 0) {
                ...
                var load = '<div id="wayne-tab-loading' + index + '" class="wayne-tab-loading">' +
                    '<div class="ball-loader">' +
                    '<span></span><span></span><span></span><span></span>' +
                    '</div>' +
                    '</div>'
                $("#" + elem).find(".wayne-tab").append(load);
                var wayneLoad = $("#" + elem).find("#wayne-tab-loading" + index);
                wayneLoad.css({
                    display: "block"
                });
                setTimeout(function () {
                    wayneLoad.fadeOut(500);
                }, time);
                index++;
            }
            element.tabAdd(elem, {
            ...
            tabData.push(opt);
            sessionStorage.setItem(elem + "-wayne-tab-data", JSON.stringify(tabData));
        } else {
            var isData = false;
        ...
        sessionStorage.setItem(elem + "-wayne-tab-data-current", opt.id);
        element.tabChange(elem, opt.id);
    }

    /** 添 加 唯 一 选 项 卡 */
    wayneTab.prototype.addTabOnly = function (opt, time) {
        var title = '';
        if (opt.close) {
            title += '<span class="wayne-tab-active"></span><span class="able-close">' + opt.title +
                '</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
        ...
        if ($(".layui-tab[lay-filter='" + this.option.elem + "'] .layui-tab-title li[lay-id]").length <= 0) {
            if (time != false && time != 0) {
                var load = '<div id="wayne-tab-loading' + index + '" class="wayne-tab-loading">' +
                    ...
                    '</div>' +
                    '</div>';
                $("#" + this.option.elem).find(".wayne-tab").append(load);
                var wayneLoad = $("#" + this.option.elem).find("#wayne-tab-loading" + index);
                wayneLoad.css({
                    display: "block"
                });
                setTimeout(function () {
    ...
    // 刷 新 指 定 的 选 项 卡
    wayneTab.prototype.refresh = function (time) {
        // 刷 新 指 定 的 选 项 卡
        if (time != false && time != 0) {
            var load = '<div id="wayne-tab-loading' + index + '" class="wayne-tab-loading">' +
                '<div class="ball-loader">' +
            ...
            index++;
            setTimeout(function () {
                wayneLoad.fadeOut(500, function () {
                    wayneLoad.remove();
                });
            }, time);
    ...
    function tabDelete(elem, id, callback, option) {
        ...
        if (!removeTab.hasClass("layui-this")) {
            ...
            tabData = JSON.parse(sessionStorage.getItem(elem + "-wayne-tab-data"));
            tabDataCurrent = sessionStorage.getItem(elem + "-wayne-tab-data-current");
        ...
        sessionStorage.setItem(elem + "-wayne-tab-data", JSON.stringify(tabData));
        sessionStorage.setItem(elem + "-wayne-tab-data-current", currId);
    ...
    function createTab(option) {
        ...
        var tab = '<div class="wayne-tab ' + types + type + ' layui-tab" lay-filter="' + option.elem +
            '" lay-allowClose="true">';
        ...
        $.each(option.data, function (i, item) {
            ...
            if (option.index == index) {
                TitleItem += '<li lay-id="' + item.id + '" class="layui-this"><span class="wayne-tab-active"></span>';
            } else {
                TitleItem += '<li lay-id="' + item.id + '" ><span class="wayne-tab-active"></span>';
            }
